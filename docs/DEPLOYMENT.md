...
`develop`에 push되면 GitHub Actions가 자동으로 테스트 → 빌드 → 배포까지 진행합니다 (`.github/workflows/deploy.yml`).
...
수동으로 다시 배포하고 싶을 때는 GitHub 저장소 Actions 탭에서 `Deploy to EC2` 워크플로우를 `workflow_dispatch`로 직접 실행할 수 있습니다.
